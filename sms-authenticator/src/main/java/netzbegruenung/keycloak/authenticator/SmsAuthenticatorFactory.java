...
public class SmsAuthenticatorFactory implements AuthenticatorFactory {
	...
	public List<ProviderConfigProperty> getConfigProperties() {
		return List.of(
			...
			new ProviderConfigProperty("senderIdOverrides", "SenderId per country", "Overrides the SenderId for the destination countries listed, one rule per entry, written as"
				+ " <countries>=<sender>. Countries are ISO 3166-1 alpha-2 codes (US, CA, ES), several per rule separated by commas; a dialling prefix (+1) can be used instead."
				+ " Example: US,CA=18885551234, for a Vonage toll-free number that US and Canadian networks require. Country rules are checked before prefix rules and the longest prefix wins; without a match 'SenderId' is used. Numbers are matched most reliably when they are stored in E.164 form."
				+ " With a 'Request JSON template', the template must contain the {senderId} placeholder for the override to apply.", ProviderConfigProperty.MULTIVALUED_STRING_TYPE, Collections.emptyList()),
			...
			new ProviderConfigProperty("jsonTemplate", "Request JSON template", "Custom JSON Body template with \"%s\" placeholders for UUID (if 'Use message UUID' is set), phone number and message (in that order), plus an optional {senderId} placeholder that receives the sender for the destination country. If empty, default template will be used.", ProviderConfigProperty.TEXT_TYPE, ""),
